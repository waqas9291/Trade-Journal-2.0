...
import { Send, Image as ImageIcon, Trash2, Key, Bot, User, Loader2, BarChart2, X } from 'lucide-react';
...
export const AIAnalyst: React.FC<AIAnalystProps> = ({ trades }) => {
    const [apiKey, setApiKey] = useState(() => localStorage.getItem('mr_wick_api_key') || '');
    const [messages, setMessages] = useState<Message[]>(() => {
        const saved = localStorage.getItem('mr_wick_chat_history');
        return saved ? JSON.parse(saved) : [{
            id: 'intro',
            role: 'model',
            text: "Hello. I am Mr. Wick. I'm here to analyze your trades, review your charts, and sharpen your edge. Send me a chart or ask me to review your journal."
        }];
    });
    ...
    // Persist Data
    useEffect(() => {
        localStorage.setItem('mr_wick_api_key', apiKey);
    }, [apiKey]);

    useEffect(() => {
        localStorage.setItem('mr_wick_chat_history', JSON.stringify(messages));
    }, [messages]);
    ...
    const clearChat = () => {
        if(window.confirm("Clear chat history?")) {
            setMessages([{
                ...
                role: 'model',
                text: "Memory wiped. We start fresh. What do you need?"
            }]);
            localStorage.removeItem('mr_wick_chat_history');
        }
    };

    const constructHistory = (): Content[] => {
        return messages.filter(m => !m.isError && m.id !== 'intro').map(m => {
    ...
    const sendMessage = async (overrideText?: string, hiddenContext?: string) => {
        ...
        try {
            ...
            const systemInstruction = "You are 'Mr. Wick', a professional, disciplined, and slightly intense trading mentor. You focus on risk management, psychology, and price action. Keep answers concise, actionable, and stern but helpful.";

            // 1. Initialize Chat with History
            // We need to be careful with history. If the history is too long, it might break. 
            // For now, we pass previous messages.
            const history = constructHistory();
            
            // Remove the just-added message from history (since we send it now)
            // Actually, constructHistory filters based on state, which we just updated. 
            // But state update is async-ish in React logic, but here inside function it's not reflected yet in 'messages' variable unless we use prev.
            // So 'constructHistory' uses the old 'messages'. That is perfect for the 'history' param.
            ...
            if (newMessage.image) {
                ...
                const data = newMessage.image.split(',')[1];
                parts.push({ inlineData: { mimeType, data } });
            }

            // Add Text
            let finalPrompt = newMessage.text;
            if (hiddenContext) {
            ...
            setMessages(prev => [...prev, {
                id: crypto.randomUUID(),
                role: 'model',
                text: "I encountered an error connecting to the secure line. Check your API Key.",
                isError: true
            }]);
        } finally {
            setIsLoading(false);
        ...
        <div className="flex flex-col h-[calc(100vh-140px)] md:h-[calc(100vh-100px)] max-w-5xl mx-auto bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xl overflow-hidden">
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center shrink-0">
                <div className="flex items-center space-x-3">
                    ...
                    <div>
                        <h2 className="font-bold text-slate-900 dark:text-white">Mr. Wick</h2>
            ...
            {showKeyInput && (
                <div className="p-4 bg-slate-100 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-700 shrink-0 animate-in slide-in-from-top-2">
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-2 uppercase">Google Gemini API Key Required</label>
                    <div className="flex gap-2">
                        <input 
                            type="password" 
                            value={apiKey}
                            onChange={(e) => setApiKey(e.target.value)}
                            className="flex-1 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white focus:ring-1 focus:ring-gold-500 outline-none"
                            placeholder="sk-..."
                        />
                        <button 
                ...
                {messages.map((msg) => (
                    <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`flex max-w-[85%] md:max-w-[75%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                            {/* Avatar */}
                            <div className={`shrink-0 h-8 w-8 rounded-full flex items-center justify-center mt-1 mx-2 ${msg.role === 'user' ? 'bg-slate-200 dark:bg-slate-700' : 'bg-gold-500/10'}`}>
                                {msg.role === 'user' ? <User className="h-5 w-5 text-slate-500 dark:text-slate-400" /> : <Bot className="h-5 w-5 text-gold-600 dark:text-gold-500" />}
                            </div>
                                ...
                                <div className="prose prose-sm dark:prose-invert max-w-none">
                                     {msg.text.split('\n').map((line, i) => (
                                         <p key={i} className="min-h-[1.2em] mb-1 last:mb-0">
                                            {line.startsWith('**') ? <strong className={msg.role === 'model' ? "text-slate-900 dark:text-white" : ""}>{line.replace(/\*\*/g, '')}</strong> : line.replace(/\*\*/g, '')}
                                         </p>
                                     ))}
                                </div>
                            </div>
                ...
                <div className="flex gap-2">
                    ...
                    <div className="flex-1 relative">
                        <input
                            ...
                            className="w-full h-full bg-slate-100 dark:bg-slate-900 border-transparent focus:bg-white dark:focus:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-slate-900 dark:text-white focus:ring-2 focus:ring-gold-500 outline-none transition-all"
                        />
                    </div>
                    ...
                    <button 
                        ...
                        className="p-3 bg-gold-600 hover:bg-gold-500 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-xl shadow-lg shadow-gold-600/20 transition-all transform active:scale-95"
                    >
