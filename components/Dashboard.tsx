...
export const Dashboard: React.FC<DashboardProps> = ({ trades, accountBalance }) => {
    const stats = useMemo(() => {
        ...
        // Mocking streak for visuals
        const streak = 4; 

        return { netPnl, profitFactor, winRate, tradeCount: closed.length, streak };
    }, [trades]);

    // Data for the radar chart (Zella Score equivalent)
    const radarData = [
    ...
    return (
        <div className="p-6 max-w-[1600px] mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <MetricCard label="Net P&L" value={`$${stats.netPnl.toLocaleString()}`} color="emerald" info="Total profit after fees" />
                <MetricCard label="Profit Factor" value={stats.profitFactor.toFixed(2)} color="indigo" info="Gross Profit / Gross Loss" isCircle />
                        ...
                        <div className="h-48 w-full">
                            <ResponsiveContainer width="100%" height="100%">
                                <RadarChart cx="50%" cy="50%" outerRadius="70%" data={radarData}>
                                    ...
                                    <PolarAngleAxis dataKey="subject" fontSize={8} fontWeight="bold" />
                                    <Radar name="Trader" dataKey="A" stroke="#6366f1" fill="#6366f1" fillOpacity={0.5} />
                                </RadarChart>
                            </ResponsiveContainer>
                        </div>
                        <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center">
                           <span className="text-xs font-bold text-slate-400 uppercase">Your Terminal Score:</span>
...
const MetricCard = ({ label, value, color, info, isCircle }: any) => (
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm flex flex-col justify-between group">
        <div className="flex justify-between items-center">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{label}</span>
            <Info className="h-3 w-3 text-slate-300 group-hover:text-slate-500 transition-colors" />
        </div>
